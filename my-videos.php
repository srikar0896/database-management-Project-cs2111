...
echo '<center>';
$show=$_SESSION['user_name'].' DATABASE';
echo '</center>';
echo '<br><br>';
...
if(isset($_POST['name'])){$target_dir = "videos/";
echo '<br>';
$target_file = $target_dir . $_POST['name'].".mp4";
...
$check = $_FILES['file']['tmp_name'];
	...
	$uploading_query="INSERT INTO ".$mysql_db.".`videos` (`id`, `data`, `data_link`, `view`) VALUES (NULL, '".$_POST['name']."', '".$_POST['name'].".mp4', '".$_POST['view']."');";
    mysql_query($uploading_query);}
	}


$query="SELECT `data`,`data_link` FROM `videos` WHERE `view`='1' ORDER By `id` DESC LIMIT 0,3";
...
while($result_array=mysql_fetch_array($query_run,MYSQL_ASSOC)){
   ...
   foreach ($channel as $element => $content) {
	   $url="videos/".$result_array['data_link'];
      echo " <$element>";
...
echo '</center>';
$query1="SELECT `data`,`data_link` FROM `videos` WHERE `view`='0' ORDER By `id` DESC LIMIT 0,3";
